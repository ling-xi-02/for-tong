...
// 专辑名数组
let albums = ['打上花火', 'なんでもないや', '大丈夫'];
// 歌曲信息数组
let track_names = ['DAOKO _ 米津玄師 - 打上花火', 'RADWIMPS - なんでもないや', 'RADWIMPS - 大丈夫'];
...
function selectTrack(flag) {
    ...
    // 切换溢出专辑数组时, 恢复cur_index, 设置成循环歌单播放
    if (cur_index == albums.length && flag == 1) {
        cur_index = 0;
    }
    if (cur_index == -1 && flag == -1) {
    ...
    // 设置音频路径
    audio.src = '../music/mp3/' + cur_album + '.mp3';
    if (flag != 0) {
        // 当切换上一首,下一首时,自动播放
        audio.play();
        player_track.classList.add('active');
